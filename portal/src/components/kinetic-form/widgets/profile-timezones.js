/**
 * Canonical list of IANA timezone identifiers used by the Profile widget
 * to validate the curated `values` arrays a form designer supplies for
 * the `timezone` core property.
 *
 * Prefers the runtime list from `Intl.supportedValuesOf('timeZone')`
 * when the host environment supports it (Chrome 99+, Firefox 93+,
 * Safari 15.4+). Falls back to a comprehensive static list for older
 * environments and to keep validation deterministic across browsers
 * during local development.
 *
 * The static fallback covers the common regions a typical portal
 * deployment cares about. The widget treats validation as advisory —
 * unknown entries warn at registration but still render in the select.
 */
...
const STATIC_TIMEZONES = [
  ...
  // Legacy aliases Kinetic / Java commonly still emit
...
// Build the canonical set once. Prefers the host's `Intl.supportedValuesOf`
// list when available (more accurate, picks up zones added since this
// file was last touched) and unions it with the static list so legacy
// aliases like `US/Central` continue to validate.
let canonicalSet;
const getCanonical = () => {
  ...
  canonicalSet = new Set([...runtime, ...STATIC_TIMEZONES]);
  return canonicalSet;
};

export const isKnownTimezone = id => getCanonical().has(id);
